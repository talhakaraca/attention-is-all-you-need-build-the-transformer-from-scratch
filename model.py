...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    # all_tokens = chain(specials, words)
    # unique_tokens = dict.fromkeys(all_tokens)
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    # Pre-fetch the unknown token ID once to avoid repeated dict lookups
...
def stack_padded_sequences_to_batch(padded_sequences):
    ...
    return torch.as_tensor(padded_sequences)
...
def scale_embeddings_by_sqrt_d_model(embeddings, d_model):
    ...
    return embeddings * torch.sqrt(d_model)
...
def compute_positional_div_term(d_model):
    ...
    div_term = torch.exp(-torch.log(torch.tensor(10000)) * (2*i / d_model))
...
def build_position_index_column(max_len):
    ...
    return torch.reshape(pos, (max_len, 1))
